...
const ScholarshipResults = ({ 
  ...
  // Use passed scholarships or create recommended/other split
  let recommendedScholarships = [];
  let otherScholarships = [];
  
  if (scholarships && scholarships.length > 0) {
    // If we have real scholarships data, use it directly
    recommendedScholarships = scholarships.slice(0, 2);
    otherScholarships = scholarships.slice(2);
  } else {
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 text-sm text-gray-500">
              <div className="flex items-center gap-1">
                ...
                <span>本月已有{scholarship.applicationCount || 0}人申请</span>
              </div>
              <div className="flex items-center gap-1">
                <Eye className="h-3 w-3" />
                <span>浏览量：{scholarship.viewCount || 0}+</span>
              </div>
            </div>
          </div>
      ...
      <CardContent>
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
          <p className="text-sm font-medium text-green-800">
            申请资格评估: {scholarship.eligibilityStatus || "符合申请条件"}
          </p>
        </div>
        ...
        <div className="flex justify-between items-center pt-4 border-t border-gray-200 gap-4">
          <div className="flex items-center gap-1">
            {[...Array(5)].map((_, i) => (
              <Star 
                key={i} 
                className={`h-4 w-4 ${i < Math.floor((scholarship.matchRate || 0) / 20) ? 'text-yellow-400 fill-current' : 'text-gray-300'}`} 
              />
            ))}
        ...
        {onBack && (
          <div className="flex items-center mb-8">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1">找到 {allScholarships.length} 个匹配的奖学金项目</p>
            </div>
          </div>
        )}
            ...
            <div className="grid md:grid-cols-4 gap-6 text-center">
              ...
              <div className="relative">
                <div className="text-3xl font-bold mb-2">
                  {Math.round(allScholarships.reduce((acc, s) => acc + (s.matchRate || 0), 0) / allScholarships.length)}%
                </div>
                <div className="text-blue-100 flex items-center justify-center gap-1">
                  平均匹配度
